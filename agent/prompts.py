...
- When navigating to an OBJECT (not a named checkpoint), pass
  stop_distance=0.20 so the bot stops ~20 cm short — close enough that
  the canned grasp poses reach the object. The camera can't see at this
  range due to mounting angle (don't scan again after nav). For
...
- For low-confidence detections (< 0.5), navigate_to with stop_distance
  ...
  confirmation. Then approach again with stop_distance=0.20 before
  pick_up.
